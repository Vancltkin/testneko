...
install_ipk() {
    ...
    new_version=$(echo "$response" | jq -r '.tag_name')
    ...
    download_url=$(echo "$response" | jq -r ".assets[] | select(.name | test(\"${package_name}_.*-${language_choice}_all.ipk\")) | .browser_download_url")
