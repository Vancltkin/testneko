...
install_ipk() {
    ...
    new_version=$(echo "$response" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
install_singbox() {
    ...
    mkdir -p "$temp_dir"
    tar xzvf "$local_file" -C "$temp_dir" --strip-components=2
    if [ $? -ne 0 ]; then
        echo -e "${RED}Extraction failed!${NC}"
        return 1
    fi
    ...
    if [ -e "$extracted_file" ]; then
        ...
        echo -e "${GREEN}Sing-Box installation completed! Version: ${latest_version}${NC}"
        echo "$latest_version" > /etc/neko/version_singbox.txt
    else
...
install_puernya() {
    ...
    echo -e "${CYAN}Checking for Puernya updates...${NC}"
    ...
    echo -e "${CYAN}Latest version: $(basename "$download_url")${NC}"
    ...
    if [ -e "$extracted_file" ]; then
        ...
        echo -e "${GREEN}Puernya core installation completed! Version: $(basename "$download_url")${NC}"
        echo "$(basename "$download_url")" > /etc/neko/version_puernya.txt
    else
        echo -e "${RED}The extracted file 'CrashCore' does not exist.${NC}"
        return 1
    fi
    ...
    echo -e "${GREEN}Puernya core update process completed successfully.${NC}"
}
...
install_ui() {
    ...
    echo -e "${CYAN}Checking for Metacube XD UI updates...${NC}"
    ...
    mkdir -p "$temp_extract_path"
    tar xzvf "$local_file" -C "$temp_extract_path"
    if [ $? -ne 0 ]; then
        echo -e "${RED}Extraction failed!${NC}"
        return 1
    fi
    ...
    cp -rT "$temp_extract_path/"* "$install_path/"
    if [ $? -ne 0 ]; then
    ...
    echo -e "${GREEN}Metacube XD UI update completed! Current version: $latest_version${NC}"

    rm -f "$local_file"
    rm -rf "$temp_extract_path"

    echo -e "${GREEN}Metacube XD UI update process completed successfully.${NC}"
}
